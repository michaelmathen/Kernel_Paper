...
def plot_interp(ax, x, y, sig=60, logsc=False, color=None, marker=None):
    if not x:
        return None
    i_kind = "slinear"
    mx = max(*x)
    ...
    if logsc:
        ...
        return ax.plot(numpy.exp(xp), poly(xp), color=color, marker=marker)
    else:
        poly = interp1d(x, y_f, kind=i_kind)
        xp = numpy.linspace(mn, mx, 25)
        return ax.plot(xp, poly(xp), color=color, marker=marker)
